# -*- coding: utf-8 -*-
"""
Created on Sun Mar 18 09:37:58 2018
...
Xtr = train.drop("pRegARisk",1)
Xre = rest.drop("pRegARisk",1)  
min_max_scaler = preprocessing.MinMaxScaler()
tr_scaled = min_max_scaler.fit_transform(Xtr)
re_scaled = min_max_scaler.fit_transform(Xre)
train = np.hstack((np.reshape(train["pRegARisk"].values,(16000,1)),pd.DataFrame(tr_scaled).values))   
rest = np.hstack((np.reshape(rest["pRegARisk"].values,(2000,1)),pd.DataFrame(re_scaled).values))                   
...
y_mean =    np.mean(fdata["pRegARisk"].values)
